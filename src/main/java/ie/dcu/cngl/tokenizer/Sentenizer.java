    ...
    private Sentenizer(Tokenizer tokenizer) {
	    ...
	    try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(
                            Sentenizer.class.getResourceAsStream(TokenizerUtils.badSentenceStart)));
	    ...
	    try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(
                            Sentenizer.class.getResourceAsStream(TokenizerUtils.possibleSentenceEnd)));
	    ...
	    try {
            BufferedReader reader = new BufferedReader(new InputStreamReader(
                            Sentenizer.class.getResourceAsStream(TokenizerUtils.badSentenceEnd)));
	        while (null != (line = reader.readLine())) {
				if (line.equals(StringUtils.EMPTY) || line.startsWith(TokenizerUtils.COMMENT)) {
					;
				} else if (line.length() < 3) {
